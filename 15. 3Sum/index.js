...
var threeSum = function (nums) {
    const result = {};
    const twoSum = (nums, target, before) => {
        const temp = new Map();
        const result = [];
        for (let i = 0; i < nums.length; i++) {
            if (i === before) continue;
            const restNumber = target - nums[i];
            if (temp.has(restNumber)) {
                result.push([temp.get(restNumber), i]);
            }
            temp.set(nums[i], i);
        }
        return result;
    ...
    for (let i = 0; i < nums.length; i++) {
        const restNumber = 0 - nums[i];
        const cases = twoSum(nums, restNumber, i);
        for (let twoCase of cases) {
            const [j, k] = twoCase;
            if (!j || !k) continue;
            ...
            const uniqueKey = item.sort().join('');
            if (!result[uniqueKey]) {
